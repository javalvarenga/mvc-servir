...
export default function EditarOrdenCompraPage() {
  ...
  const fetchOrdenCompra = useCallback(async () => {
    try {
      setLoadingData(true)
      const response = await fetch(`/api/ordenes-compra?id=${params?.id}`)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const confirmed = await confirmAction(
      '¿Guardar cambios?',
      '¿Estás seguro de que quieres guardar los cambios realizados a la orden de compra?',
      'Sí, guardar'
    )
    ...
    try {
      ...
      if (data.success) {
        showSuccess('¡Guardado!', 'La orden de compra ha sido actualizada exitosamente')
        router.push('/proyectos')
      } else {
        showError('Error', data.message || 'Error al actualizar la orden de compra')
      }
    } catch (err) {
      closeLoading()
  ...
  const handleDelete = async () => {
    ...
    const confirmed = await confirmDelete(
      ...
      `¿Estás seguro de que quieres eliminar la orden de compra "${ordenCompra.numeroOrden}" por Q${ordenCompra.monto}? Esta acción no se puede deshacer.`,
      'orden de compra'
    )
    ...
    try {
      showLoading('Eliminando...', 'Por favor espera mientras eliminamos la orden de compra')
      ...
      if (data.success) {
        showSuccess('¡Eliminado!', 'La orden de compra ha sido eliminada exitosamente')
        router.push('/proyectos')
      } else {
        showError('Error', data.message || 'Error al eliminar la orden de compra')
      }
    } catch (err) {
      closeLoading()
      showError('Error de conexión', 'No se pudo conectar con el servidor')
    ...
    return (
      <div className="px-4 py-6 sm:px-0">
        <div className="text-center">
          <h1 className="text-2xl font-semibold text-gray-900">Orden de compra no encontrada</h1>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-gray-600">
                {ordenCompra.numeroOrden} - {ordenCompra.proveedor}
              </p>
            </div>
